...
public class WifiIotPlugin implements FlutterPlugin, ActivityAware, MethodCallHandler, EventChannel.StreamHandler {
    ...
    private final List<String> ssidsToBeRemovedOnExit = new ArrayList<>();
    private final List<WifiNetworkSuggestion> suggestionsToBeRemovedOnExit = new ArrayList<>();
    ...
    private void isSSIDHidden(Result poResult) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            android.net.wifi.WifiConfiguration wiFiConfig = moWiFiAPManager.getWifiApConfiguration();
            ...
            poResult.success(wiFiConfig.hiddenSSID);
        } else {
    ...
    private void setSSIDHidden(MethodCall poCall, Result poResult) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            ...
            oWiFiConfig.hiddenSSID = isSSIDHidden != null && isSSIDHidden;
     ...
     * double quotation marks (e.g., {@code "passphrase"} for PSK passphrase or
     ...
     * *** getClientList :
     * param onlyReachable   {@code false} if the list should contain unreachable (probably disconnected) clients, {@code true} otherwise
     * param reachableTimeout Reachable Timeout in milliseconds, 300 is default
     * param finishListener,  Interface called when the scan method finishes
     */
    private void getClientList(MethodCall poCall, final Result poResult) {
        Boolean onlyReachable = false;
        if (poCall.argument("onlyReachable") != null) {
            onlyReachable = poCall.argument("onlyReachable");
        }
        ...
        final Boolean finalOnlyReachable = onlyReachable;
            ...
            public void onFinishScan(final ArrayList<ClientScanResult> clients) {
                try {
                    ...
                    for (ClientScanResult client : clients) {
                        ...
                        if (finalOnlyReachable != null && finalOnlyReachable) {
                            if (!clientIsReachable) {
                                shouldReturnCurrentClient = Boolean.FALSE;
                            }
                        }
                        if (shouldReturnCurrentClient) {
                            try {
        ...
        final boolean safeOnlyReachable = onlyReachable != null && onlyReachable;
        if (reachableTimeout != null) {
            moWiFiAPManager.getClientList(safeOnlyReachable, reachableTimeout, oFinishScanListener);
        } else {
            moWiFiAPManager.getClientList(safeOnlyReachable, oFinishScanListener);
        }
    }
    ...
    private void setWiFiAPEnabled(MethodCall poCall, Result poResult) {
        ...
        boolean safeEnabled = enabled != null && enabled;
        
        // Using LocalOnlyHotspotCallback when setting WiFi AP state on API level >= 29
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            moWiFiAPManager.setWifiApEnabled(null, safeEnabled);
        } else {
            if (safeEnabled) {
                moWiFi.startLocalOnlyHotspot(new WifiManager.LocalOnlyHotspotCallback() {
                    ...
                    public void onFailed(int reason) {
                        ...
                        Log.d(WifiIotPlugin.class.getSimpleName(), "LocalHotspot failed with code: " + reason);
                    }
                }, new Handler());
            } else {
    ...
    private void showWritePermissionSettings(MethodCall poCall, Result poResult) {
        ...
        boolean safeForce = force != null && force;
        moWiFiAPManager.showWritePermissionSettings(safeForce);
        poResult.success(null);
    }
    ...
    private void forceWifiUsage(final MethodCall poCall, final Result poResult) {
        ...
        boolean safeUseWifi = useWifi != null && useWifi;
    ...
    private void setEnabled(MethodCall poCall, Result poResult) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            moWiFi.setWifiEnabled(safeEnabled);
        }
    ...
    private void getIP(Result poResult) {
        WifiInfo info = moWiFi.getConnectionInfo();
        String ip = longToIP(info.getIpAddress());
        poResult.success(ip);
    }
    ...
    private android.net.wifi.WifiConfiguration generateConfiguration(String ssid, String bssid, String password, String security, Boolean isHidden) {
        ...
        if (security.toUpperCase().equals("WPA")) {
            ...
            // Appropriate cipher is needed depending on the security type used.
            // In case of not added it will not be able to connect.
