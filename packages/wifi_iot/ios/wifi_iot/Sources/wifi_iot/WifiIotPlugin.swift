...
public class WifiIotPlugin: NSObject, FlutterPlugin {
  ...
  private func connect(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    if #available(iOS 11.0, *) {
      let configuration = initHotspotConfiguration(
        ssid: sSSID, passphrase: sPassword, security: sSecurity)
      ...
      NEHotspotConfigurationManager.shared.apply(configuration) { [weak self] (error) in
        ...
        this.getSSID { (connectedSSID) -> Void in
          if error != nil {
  ...
  @available(iOS 11.0, *)
  private func initHotspotConfiguration(ssid: String, passphrase: String?, security: String? = nil)
  ...
  private func getBSSID(result: @escaping (String?) -> Void) {
    if #available(iOS 14.0, *) {
      NEHotspotNetwork.fetchCurrent(completionHandler: { currentNetwork in
